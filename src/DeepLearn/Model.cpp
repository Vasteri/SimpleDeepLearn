...
std::vector<double> Model::matrix_to_vector(const Matrix<double>& input_matrix){
    ...
    for (int i = 0; i < input_matrix.GetMemory(); i++)
        result_vector.push_back(input_matrix[i]);
...
std::vector<double> Model::fit_chaotic(std::vector<std::vector<double>>& x, std::vector<std::vector<double>>& y_true, int batch_size, int n){
