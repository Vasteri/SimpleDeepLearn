...
double approx_function(double a, double b) { return (-7) * a + (-2) * b + (7); }
...
int main() {
    ...
    for (unsigned int i = 0; i < n; i++) {
        double a = (double)(std::rand() % 100 - 50) / 50;
        double b = (double)(std::rand() % 100 - 50) / 50;
        x[i].push_back(a);
        x[i].push_back(b);
        double r = approx_function(a, b);
